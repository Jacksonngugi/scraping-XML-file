...
def admin():
    ...
    while (1):
        ...
        print("7) Products that needs to be restocked")
        print("8).View demamd of product(s) in the future")
        print("9)Exit")
...
def warning():
    ...
    item = []       # format later
    for i in data.keys():
        if float(data[i]['quantity']) < 10:
           item.append(data[i]['name'])

    if item == []:
        print("NO ITEM TO BE RESTOCKED")  
...
def predict():
    ...
    table = table[['time_date','name','product_id','quantity']]
    train = table
    ...
    train[x] =lb.fit_transform(train[x])
    train[y] =lb.fit_transform(train[y])
    ...
    clf = xgb_r.fit(train[x],train[y])

    print("Enter product_id")

    product_id =int(input())

    #pred =lb.transform(product_id)

    result =clf.predict(product_id)
